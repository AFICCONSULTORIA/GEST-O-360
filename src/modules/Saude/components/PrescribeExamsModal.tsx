...
import { 
  Patient, HealthUnit, HealthProfessional, ExamRequest, ExamCategory,
...
const COMMON_SUS_EXAMS: Array<{ name: string; category: ExamCategory }> = [
  { name: 'Hemograma Completo', category: 'Laboratorial' },
  { name: 'Glicemia de Jejum', category: 'Laboratorial' },
  { name: 'Lipidograma Completo (Colesterol e Triglicerídeos)', category: 'Laboratorial' },
  { name: 'Urina Tipo I (EAS) e Urocultura', category: 'Laboratorial' },
  { name: 'Ureia e Creatinina (Função Renal)', category: 'Laboratorial' },
  { name: 'TSH e T4 Livre (Tireoide)', category: 'Laboratorial' },
  { name: 'Hemoglobina Glicada (HbA1c)', category: 'Laboratorial' },
  { name: 'Ácido Úrico', category: 'Laboratorial' },
  { name: 'Parasitológico de Fezes (EPF)', category: 'Laboratorial' },
  { name: 'Raio-X de Tórax (PA e Perfil)', category: 'Imagem' },
  { name: 'Eletrocardiograma (ECG)', category: 'Cardiológico' },
  { name: 'Ultrassonografia de Abdome Total', category: 'Imagem' },
  { name: 'Sorologias (HIV, Sífilis, Hepatites B e C)', category: 'Laboratorial' },
  { name: 'Mamografia Bilateral', category: 'Imagem' }
];
...
export const PrescribeExamsModal: React.FC<PrescribeExamsModalProps> = ({
  ...
  const handleToggleExam = (name: string, category: ExamCategory) => {
    ...
    if (existingIndex >= 0) {
      ...
      const dup = checkExamDuplicity(patient.cpf, name, allExamRequests);
  ...
  const handleAddCustomExam = (e?: React.FormEvent) => {
    ...
    const dup = checkExamDuplicity(patient.cpf, trimmed, allExamRequests);
    setSelectedExams(prev => [
      ...prev,
      {
        id: generateUUID(),
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm overflow-y-auto">
      ...
      >
        ...
        <div className="p-6 overflow-y-auto space-y-5 flex-1 custom-scrollbar text-neutral-800 dark:text-neutral-200">
          ...
          {/* Seleção Rápida de Exames do SUS (Chips) */}
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <label className="text-xs font-black uppercase tracking-wider text-neutral-600 dark:text-neutral-300 flex items-center gap-1.5">
                <Activity size={14} className="text-blue-600" /> Exames Mais Solicitados (Clique para marcar)
              </label>
              <span className="text-[11px] text-neutral-400 font-medium">
                {selectedExams.length} exame{selectedExams.length === 1 ? '' : 's'} marcado{selectedExams.length === 1 ? '' : 's'}
              </span>
            </div>

            <div className="flex flex-wrap gap-1.5">
              {COMMON_SUS_EXAMS.map(item => {
                const isSelected = selectedExams.some(e => e.name.toLowerCase() === item.name.toLowerCase());
                return (
                  <button
                    key={item.name}
                    type="button"
                    onClick={() => handleToggleExam(item.name, item.category)}
          ...
          {/* Adicionar Outro Exame / Busca */}
          <div className="space-y-2 bg-neutral-50 dark:bg-neutral-800/50 p-3.5 rounded-2xl border border-neutral-200/70 dark:border-neutral-800">
            <label className="text-xs font-bold text-neutral-700 dark:text-neutral-300 flex items-center gap-1.5">
              <Search size={13} className="text-neutral-500" /> Adicionar Outro Exame Não Listado
            </label>
            <div className="flex flex-wrap sm:flex-nowrap gap-2">
              <input
                ...
                placeholder="Ex: Ecocardiograma Transtorácico, PSA Total, Citopatológico..."
                className="flex-1 px-3 py-2 bg-white dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 text-xs focus:ring-2 focus:ring-blue-500 outline-none"
              />
              <select
                value={customExamCategory}
                onChange={e => setCustomExamCategory(e.target.value as ExamCategory)}
                className="px-3 py-2 bg-white dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 text-xs font-medium focus:ring-2 focus:ring-blue-500 outline-none"
              >
              ...
              <button
                ...
                className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold transition-colors flex items-center gap-1 cursor-pointer shrink-0"
              >
                      ...
                      {isDup && (
                        <div className="mt-2.5 pt-2 border-t border-amber-200 dark:border-amber-800/50 space-y-2">
                          <div className="flex items-start gap-1.5 text-amber-800 dark:text-amber-300 text-[11px]">
                            ...
                            <span>
                              <strong>Aviso Anti-Duplicidade SUS:</strong> Este paciente realizou este exame há{' '}
                              <strong>{item.duplicityResult?.daysSince} dia(s)</strong> em{' '}
                              {lastDate ? lastDate.split('-').reverse().join('/') : 'data recente'} (Intervalo mínimo recomendado: 30 dias).
                            </span>
                          </div>
