...
export const Login = ({ onLogin, onDemoLogin, darkMode, setDarkMode, currentInstitution, isSaaSAdmin = false }: { onLogin: () => void, onDemoLogin?: () => void, darkMode: boolean, setDarkMode?: (v: boolean) => void, currentInstitution?: Institution | null, isSaaSAdmin?: boolean }) => {
  ...
  return (
    <div className="dark min-h-[100dvh] relative flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute top-6 left-8 flex items-center gap-3 z-20">
        <div className="p-2.5 bg-neutral-900/5 dark:bg-white/10 backdrop-blur-md border border-neutral-900/10 dark:border-white/20 rounded-xl flex items-center justify-center">
          <LogoCompass size={24} className="text-neutral-900 dark:text-white" />
        </div>
        <div>
      ...
      >
        <div className="bg-white/70 dark:bg-neutral-950/70 backdrop-blur-2xl border border-white/40 dark:border-white/10 rounded-[32px] shadow-[0_32px_80px_rgba(0,0,0,0.1)] dark:shadow-[0_32px_80px_rgba(0,0,0,0.7)] overflow-hidden transition-colors duration-500">
          
          {/* Linha decorativa no topo */}
          <div className="h-[3px] w-full bg-gradient-to-r from-emerald-500 via-teal-400 to-indigo-500" />

          <div className="px-8 pb-8 pt-8 sm:px-10 sm:pb-10 space-y-7">

            {/* Cabeçalho do card */}
            <div className="text-center space-y-1">
              {isSaaSAdmin ? (
                <>
                  <div className="inline-flex items-center justify-center w-14 h-14 bg-purple-500/10 border border-purple-500/20 rounded-2xl mb-3">
                    <Shield size={28} className="text-purple-600 dark:text-purple-400" />
                  </div>
                  <h1 className="text-2xl font-black text-neutral-900 dark:text-white tracking-tight">Central SaaS</h1>
                  <p className="text-[10px] font-bold text-purple-600 dark:text-purple-400 uppercase tracking-widest">Controle Geral de Prefeituras</p>
                </>
              ) : currentInstitution ? (
                <>
                  <div className="inline-flex items-center justify-center w-14 h-14 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl mb-3">
                    <LogoCompass size={32} className="text-emerald-600 dark:text-emerald-400" />
                  </div>
                  <h1 className="text-xl font-black text-neutral-900 dark:text-white tracking-tight leading-tight">{currentInstitution.name}</h1>
                  <p className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">Painel Administrativo Oficial</p>
                </>
              ) : (
                <>
                  <div className="inline-flex items-center justify-center w-14 h-14 bg-neutral-900/5 dark:bg-white/5 border border-neutral-900/10 dark:border-white/10 rounded-2xl mb-3">
                    <LogoCompass size={32} className="text-neutral-900 dark:text-white" />
                  </div>
                  <h1 className="text-2xl font-black text-neutral-900 dark:text-white tracking-tight">Bem-vindo de volta</h1>
                  <p className="text-sm text-neutral-500 dark:text-white/50 font-medium">Entre com suas credenciais para acessar o sistema</p>
                </>
              )}
            </div>
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-black uppercase tracking-widest text-neutral-500 dark:text-white/40 ml-1">E-mail</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    autoComplete="email"
                    className="w-full bg-neutral-900/5 dark:bg-white/5 hover:bg-neutral-900/10 dark:hover:bg-white/8 border border-neutral-900/10 dark:border-white/10 focus:border-emerald-500/60 focus:ring-4 focus:ring-emerald-500/10 px-12 py-3.5 rounded-2xl text-sm outline-none transition-all font-medium text-neutral-900 dark:text-white placeholder-neutral-400 dark:placeholder-white/25"
                    value={username}
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between items-center px-1">
                  <label className="text-[10px] font-black uppercase tracking-widest text-neutral-500 dark:text-white/40">Senha</label>
                  <span className="text-[10px] text-neutral-400 dark:text-white/30 font-medium cursor-pointer hover:text-emerald-600 dark:hover:text-white/60 transition-colors">Esqueceu a senha?</span>
                </div>
                <div className="relative">
            ...
            {/* ── Caixinha "Cara Nova" ── */}
            <div className="flex items-start gap-3 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl backdrop-blur-sm">
              ...
              <div>
                <p className="text-xs font-black text-emerald-700 dark:text-emerald-300">Estamos de cara nova!</p>
                <p className="text-[11px] text-neutral-600 dark:text-white/60 font-medium leading-relaxed mt-0.5">
          ...
          {/* Rodapé do card */}
          <div className="px-10 pb-6 text-center border-t border-neutral-900/5 dark:border-white/5 pt-4">
            <p className="text-[10px] text-neutral-400 dark:text-white/30 font-medium">
